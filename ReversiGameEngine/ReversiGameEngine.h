...
public:
	ReversiGameEngine();
	virtual ~ReversiGameEngine();

	// SETUP

	// setView
	// Saves a pointer to the instantiated view object.
	virtual void setView(ReversiGameView* view);
...
protected:
	...
	// Returns a ReversiGameStatus enum value representing update status.
	...
	virtual ReversiGameStatus updateState(const std::string& position);
